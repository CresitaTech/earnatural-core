$(document).ready(function () {
	...
	var shippingCharge = getParameterByName('shippingCharge');

	$("#shippingChargeDisplay").html('$'+shippingCharge);
	...
	console.log(promocode)
		...
		if(promocode != 'null'){
			...
			.then(function (response) {
				...
				console.log('promocodes Data res: ', res);

				console.log('promocodes Data promocode: ', res.data.promocode);

				console.log('promocodes Data promocode_discount: ', res.data.promocode_discount);

				console.log('promocodes Data promocode_type: ', res.data.promocode_type);



				if(res.data.promocode_type=='Percentage'){

					$('#total_amount_display').html('Discounted Price : $' + (unitPrice * quantity - Math.round(parseFloat(unitPrice * quantity * (res.data.promocode_discount/100))) ).toFixed(2));
	...
	$('#buttonAddress').click(function () {
		...
		if (instance.isValid()) {
			...
			var fields = promocode.split('-');

			if(fields[1]=='Percentage'){

				unitPrice = (unitPrice * quantity - Math.round(parseFloat(unitPrice * quantity * 0.33)) ).toFixed(2);

			}else{

				unitPrice = (unitPrice * quantity - Math.round(parseFloat(fields[0])) ).toFixed(2);
			...
			axios.post('/api/product', JSON.stringify(postData))
