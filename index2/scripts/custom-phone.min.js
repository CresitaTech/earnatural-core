			...
			axios.post('/api/makepayment', JSON.stringify(postData))
				...
				.then((response) => {
					...
					if (response.data.status == '201') {
						...
						window.location.href = `success.html?status=Success&message=${encodeURIComponent(response.data.messages.success)}`;
						...
						window.location.href = `error.html?status=Failed&message=${encodeURIComponent(response.data.messages.success)}`;
					...
					window.location.href = `error.html?status=Failed&message=${encodeURIComponent('Cannot connect to the server. please try again after some time.')}`;
